    ...
    try {
        ...
        if(!fruit) {
            return res.status(400).send(`
                <center>
                    <h1 style="font-size:20px;margin-bottom: 0;margin-top: 100px">Fruit not found!</h1>
                    <br>
                    If you would like to add it, send a POST request to <code><strong>/fruits/add/${name}</strong></code> with the fruit details in the body
                </center>
            `);
        }
        res.send(fruit);
...
app.get("/fruits/id/:id", async (req, res) => {
    ...
    if(isNaN(id) || id <= 0) {
        return res.status(404).send(`
            <center>
                <h1 style="font-size:20px;margin-bottom: 0;margin-top: 100px">Invalid ID</h1>
                <br>
                <p>ID must be an integer greater than zero!</p>
            </center>
        `);
    }
    try {
        ...
        if(!fruit) {
            return res.status(404).send(`
                <center>
                    <h1 style="font-size:20px;margin-bottom: 0;margin-top: 100px">Fruit not found!</h1>
                    <br>
                    <p>There is no fruit with the id ${id}</p>
                </center>
            `);
        }
...
app.get("/fruits/family/:family", async (req, res) => {
    ...
    try {
        ...
        if(fruit.length === 0) {
            return res.status(404).send(`
                <center>
                <h1 style="font-size:20px;margin-bottom: 0;margin-top: 100px">There are no fruits in the database in the "${family}" family.</h1>
                </center>
            `);
        }
        res.send(fruit);
